...
Let's now compare the two Confusion matrices:
...
The plotted confusion matrices highlight the similarity in predictions between the models. Notably, the (balanced) accuracy of the offline model is slightly higher than that of the online model. However, keep in mind that while accuracy is an important measure for assessing a model, it is not the sole factor in determining its suitability.

Additionally, the similarity in performance between the models is further evidenced by their Brier scores, which is defined as:

$$BS = \frac{1}{M} \sum_{m=1}^{M}{\sum_{i}{(p_{mi} - d_{mi})^2}} \hspace{0.2cm}\; \hspace{0.2cm}\text{with } i\in \{H,D,A\}$$

Despite the BS being an overall measure of performance (like accuracy, though), I found it interesting to plot it matchday by matchday:
...
Following the Brier score plot, we observe that both approaches yield very similar results in terms of this predictive metric. However, it's important to note that, in general, these quantitative measures do not provide the full picture. The ideal approach is to consider the entirety of our analysis, integrating various aspects, both quantitative and qualitative.

As an example, I did something interesting to evaluate and compare the models. I implemented a simulation of the final rankings. Starting from the situation at the halfway point of the season, I began simulating matches. With the model fitted after the $m$-th match, I made predictions for match $m+1$. Thanks to the Bayesian approach, I obtained a distribution of the goal differences. From this distribution, it is relatively straightforward to derive a distribution of the points. By proceeding in this manner, I obtained the distribution of the final points at the end of the season. Having a distribution allows me to provide the expected number of points at the end of the season, along with a confidence band for this result. This approach offers a comprehensive view of the potential outcomes, capturing the inherent uncertainty and variability in the two models.


To perform this task, I implemented a custom util for retrieving the final points distribution:
...
table_offline<- offline_rankings %>% 
  ...
  kbl(caption = "Serie A 2021-2022 simulated rankings (offline)", align = 'c') %>%
  kable_styling(font_size = 12, full_width = F) %>%
  row_spec(0, background = "#FFFFFF", color = "#000000",bold=T) %>%
  # Champions League
  row_spec(1:4, background = "#8AD1F3") %>% 
